...
function Element:New(Idx, Config)
	...
	local search = Create("TextBox", {
		...
		TextSize = 13,
		TextXAlignment = Enum.TextXAlignment.Left,
		ThemeProps = { BackgroundColor3 = "maincolor", PlaceholderColor3 = "descriptioncolor" },
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	function Dropdown:Set(Value, ignore)
		local function updateButtonTransparency(button, isSelected)
			...
			TweenService:Create(button, TweenInfo.new(0.15), { BackgroundTransparency = transparency }):Play()
			TweenService:Create(button.ImageLabel, TweenInfo.new(0.15), { ImageTransparency = transparency }):Play()
			TweenService:Create(button.TextLabel, TweenInfo.new(0.15), { TextColor3 = textColor }):Play()
		end
		...
		for i, button in pairs(Dropdown.Buttons) do
			local isSelected = (Config.Multiple and table.find(Dropdown.Value, i)) or (not Config.Multiple and i == Value)
...
function Element:New(Idx, Config)
	...
	local ValueText = Create("TextLabel", {
		...
		RichText = true,
		Text = "0",
		ThemeProps = { TextColor3 = "accentcolor" },
		TextSize = 14,
		TextXAlignment = Enum.TextXAlignment.Right,
		BackgroundTransparency = 1,
		Position = UDim2.new(0.75, 8, 0, 0),
	...
	function Slider:Set(Value, ignore)
		...
		if DraggingDot then
			...
			TweenService:Create(SliderDot, TweenInfo.new(0.15), { Position = UDim2.new(newPosition, 0, 0.5, 0) }):Play()
			TweenService:Create(SliderProgress, TweenInfo.new(0.15), { Size = UDim2.fromScale(newPosition, 1) }):Play()
		end
...
-- Black and Red Theme for Zvios Hub
local themes = {
	default = {
		-- Main Colors (Pure Black & Red)
		...
		descriptioncolor = Color3.fromRGB(140, 140, 140),
		elementdescription = Color3.fromRGB(110, 110, 110),
		...
		offTextBtn = Color3.fromRGB(90, 90, 90),
